...
public class UserServiceDelegateImpl implements UserServiceDelegate {
	...
	public Map findByUsername(String username) {
		return this.restTemplate.getForObject("http://localhost:8095/user" + username, Map.class);
	}
	...
	public Map findByUsername(String username) {
		...
		Map mapUser = circuitBreaker.run(
			()->this.restTemplate.getForObject("http://localhost:8095/user/" + username, Map.class),
	...
	public List<Map> findByUsernames(String usernames) {
		...
		List<Map> listMapUsers = circuitBreaker.run(
			()->this.restTemplate.getForObject("http://localhost:8095/user/list/" + usernames, List.class),
	...
	public boolean isLoggedInUser(String authToken) {
		...
		try {
			ResponseEntity<Boolean> response = 
					this.restTemplate.exchange("http://localhost:8095/token/validate",  HttpMethod.GET, entity, Boolean.class);
